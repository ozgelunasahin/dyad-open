-- Access-window guard in accept_invitation — conference-scoped access (U7).
--
-- An invitation cannot be accepted for a slot that starts after either
-- party's access window ends (plan R12): a guest must not commit to a
-- meeting they will be locked out of, and a member must not be left holding
-- a meeting whose counterpart is gone by the time it starts.
--
-- This copies the retired-slot accept_invitation body verbatim
-- (20260604150100 — the authoritative version, which itself carries the
-- capacity cap from 20260529100100) and inserts ONLY the access-window
-- guard, placed after the retired-slot branch (v_slot_start is known and
-- the slot row is locked) and before the inviter-duplicate guard. Gate A /
-- Gate B idempotency, the expired-slot branch, the retired-slot branch,
-- the capacity cap, and the REVOKE/GRANT are preserved unchanged. The
-- guard RAISEs (rather than the cancel-and-return-NULL shape of the
-- capacity branches) because the invitation itself stays valid — the
-- caller should see a distinct, mappable error, not a silent cancellation.
