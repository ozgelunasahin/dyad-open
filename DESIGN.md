...
- Responses are visible only to the conversation author. **Group conversations do not change this**: a joiner sees a count of others on a time ("+N others joining"), never identities or *response content*; the author sees who's joining (the `ParticipantsStack` roster on their offered times). Joiner-facing *presence* (first names) is tracked separately in issue #55. Letting confirmed co-participants read each other's responses was considered for the group-conversations work and **deliberately deferred** — it would reverse the one-way-message commitment above, and it discloses member-written content to an audience the writer didn't address (a `/datenschutz` surface and a consent/expectation question — see CLAUDE.md § *Data Collection and Values*), which is a founder/values call, not an engineering one. Revisit only with: (a) the Fork B usage data, (b) forward-only disclosure plus explicit consent copy (no retroactive reveal of already-written responses), and (c) a participant-scoped read path that cannot leak across conversations or before acceptance. *(Today enforced by the absence of any participant-based SELECT path on `prompt_comments`: SELECT is limited to the response's own author and the prompt author — see `20260502100400_rls_use_app_current_user_id.sql`. The analogous group-feedback reveal was likewise punted "to a founder call" in `20260529100300_group_feedback.sql`.)*
