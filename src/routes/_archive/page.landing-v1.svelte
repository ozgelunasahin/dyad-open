<script lang="ts">
	...
	onMount(() => {
		...
		function handleJoinLink(e: MouseEvent) {
			const a = (e.target as HTMLElement).closest('a[href="#join"]');
			if (a) {
				e.preventDefault();
				handleNavClick('field-notes');
			}
		}
		document.addEventListener('click', handleJoinLink);

		return () => {
			mq.removeEventListener('change', mqHandler);
			document.removeEventListener('click', handleJoinLink);
		};
	});
