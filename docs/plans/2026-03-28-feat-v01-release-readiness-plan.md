...
- [ ] **Revisit profile visual metaphor (design assessment 2026-03-29):** The action card grid borrows a photo-grid navigation pattern (stacked thumbnails). This works for conversations (cover images mandatory) but the meetings card always shows a grey placeholder — the two cards feel unbalanced. Consider whether both cards should use typographic previews (conversation title in serif, area in monospace) instead of photo stacks, matching the editorial/paper aesthetic. Also: the profile page is a hybrid of dashboard (attention section) and hub (action card navigation) without committing to either pattern. Decide which it is.
- [ ] **Sign-out mobile UX:** On mobile (no sidebar), the bare "Sign out" link orphaned at the bottom of the page feels unfinished. Replace with a `@username · Sign out` row below the profile card — gives the username presence on mobile and makes sign-out feel intentional.
- [ ] **Badge pulse urgency:** Meetings card badge currently pulses for any upcoming meeting. Consider whether pulse should only fire for imminent meetings (today/tomorrow) — a pulse that runs for weeks loses its urgency signal.
